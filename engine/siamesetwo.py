...
class LitModel(BaseModel):
    ...
    def training_step(self, batch, batch_idx=0):
        ...
        imgs = batch['img']
        labels = batch['labels']
        np.save('a.npy', np.array(batch['filenames']))
        ...
        loss0, _ = self.loss_function(output, labels)

        # R > L
        output, features = self.net((imgs[1], imgs[0]))
        loss1, _ = self.loss_function(output, 1 - labels)
    ...
    def validation_step(self, batch, batch_idx=0):
        if 1:#self.trainer.global_rank == 0:
            ...
            # L > R
            output0, features = self.net(imgs)
            loss0, _ = self.loss_function(output0, labels)

            # R > L
            output1, features = self.net((imgs[1], imgs[0]))
            loss1, _ = self.loss_function(output1, 1 - labels)
            ...
            # metrics
            self.all_label.append(torch.cat([labels, 1-labels], 0).cpu())
            self.all_out.append(torch.cat([output0, output1], 0).cpu().detach())
