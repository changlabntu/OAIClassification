...
class MRPretrainedSiamese(MRPretrained):
    ...
    def forward(self, x):  # (1, 3, 224, 224, 23)
        ...
        if self.fuse == 'mean':  # max-pooling across the slices
            x1 = torch.mean(x1, 0)  # (1, 512, 1, 1)
            x0 = torch.mean(x0, 0)  # (1, 512, 1, 1)
            x = self.avg(x1.unsqueeze(0)) - self.avg(x0.unsqueeze(0))  # (B*23, 512, 1, 1)
            out = self.classifier(x)  # (Classes)
            out = out[:, :, 0, 0]
        ...
        if self.fuse == 'max3':  # max-pooling across the slices
            x0 = nn.AdaptiveMaxPool2d((1, 1))(x0)
