...
# https://link.springer.com/content/pdf/10.1007/978-3-031-16431-6_4.pdf
# https://github.com/qbmizsj/GFNet/blob/main/gfnet_3d_3.py
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size, patch_size, num_classes=2, in_channels=1):
        super().__init__()
...
class GFNet(nn.Module):
    def __init__(self, img_size, patch_size, embed_dim, num_classes=2, in_channels=1, drop_rate=0.5, depth=8, mlp_ratio=2., representation_size=None, uniform_drop=False, drop_path_rate=0.6, norm_layer=False, dropcls=0.25):
        super().__init__()
        ...
        (h, w, z) = (a // b for a, b in zip(img_size, patch_size))
        ...
        self.blocks = nn.ModuleList([
            Block(dim=embed_dim, mlp_ratio=mlp_ratio, drop=drop_rate,
                  drop_path=dpr[i], norm_layer=norm_layer, h=h, w=w, z=z)
    ...
    def forward_features(self, x):
        if 0:
            B = x.shape[0]  # (B, 1, 181, 217, 181)
            print(x.shape)
            x = self.patch_embed(x)  # (B, 1000, 18, 21, 18)
            print(x.shape)
            x = x.flatten(2).transpose(1, 2)  # (B, 18*21*18, 1000)
        else:
        ...
        #print(x.shape)  # (B, (H*W*D/P^3), E)
        x = x + self.pos_embed  # + # (B, (H*W*D/P^3), E)
        #print(x.shape)
        x = self.pos_drop(x)
...
class Block(nn.Module):
    def __init__(self, dim, mlp_ratio=2., drop=0.5, drop_path=0.6, act_layer=nn.GELU, norm_layer=nn.LayerNorm, h=18, w=21, d=10, z=2):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.filter = Global_Filter(dim=dim, h=h, w=w, d=d, z=z)
...
class Global_Filter(nn.Module):
    def __init__(self, h, w, z, d, dim):
        super().__init__()
        self.complex_weight = nn.Parameter(torch.randn(   #  !!! weight for attention
            h, w, (z // 2) + 1, dim, 2, dtype=torch.float32) * 0.02)
        self.h = h
        self.w = w
        self.z = z
        self.dim = dim
    ...
    def forward(self, x):
        ...
        x = x.view(B, self.h, self.w, self.z, self.dim)
        x = torch.fft.rfftn(x, dim=(1, 2, 3), norm='ortho')
        weight = torch.view_as_complex(self.complex_weight)
        x = x * weight
        x = torch.fft.irfftn(x, s=(self.h, self.w, self.z), dim=(1, 2, 3), norm='ortho')
        x = x.reshape(B, N, C)
        return x


class GFSiamnese(nn.Module):
    def __init__(self):
        super().__init__()
...
if __name__ == '__main__':
    x = x = torch.randn(1, 3, 256, 256, 23)
    net = GFNet(img_size=(256, 256, 23), patch_size=(32, 32, 1), embed_dim=512)
    y = net(x)
