...
class PairedData(data.Dataset):
    ...
    def __init__(self, root, path, opt, mode, labels=None, transforms=None, filenames=False, index=None):
        ...
        if self.opt.resize == 0:
            ...
            self.resize = np.array(tiff.imread(join(self.all_path[0], self.images[0]))).shape[1]
        else:
    ...
    def load_img(self, path):
        ...
        if self.opt.trd > 0:
            ...
            #x = x / self.opt.trd
            x = (x >= self.opt.trd).astype(np.float32)
        else:
            if x.max() > 0:  # scale to 0-1
                x = x / x.max()

        if len(x.shape) == 2:  # if grayscale, get the chanel dimension
...
if __name__ == '__main__':
    ...
    if 1:
        ...
        # womac3
        opt.bysubject = True
        dataset3d = MultiData(root=root, path='effusion/aeffpainYphi_effusion/beffpainYphi', opt=opt, mode='train', filenames=False)
